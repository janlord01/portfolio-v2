  ...
  <q-page class="q-pa-md">
    <q-card-section class="q-mb-md">
      <div class="row justify-between items-center">
        <div class="text-h6 text-center">{{ settingStore.settingsData.tech_title || 'Tech Stack' }}</div>
        <q-btn
          :label="settingStore.settingsData.tech_title ? 'Add ' + settingStore.settingsData.tech_title : 'Add Experience'"
          color="primary"
          outline
          icon="add"
          to="/add-tech-stack"
...
export default {
  ...
  const onDragEnd = async (event) => {
    const reorderedtech = techs.value.map((tech, index) => ({
        id: tech.id,
        order: index + 1,
        name: tech.name,
